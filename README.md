...
| **Persistence** | Scans LaunchAgents, LaunchDaemons, login items, StartupItems, rc scripts, shell configs (incl. `.zshenv`), cron jobs, login/logout hooks, periodic scripts, Folder Actions, QuickLook / Spotlight / Audio HAL plugins |
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH server config + key hygiene, Find My Mac, sudo timeout, login window display, sharing services, software updates |
| **Supply Chain** | Audits developer toolchains — npm/yarn/pnpm lifecycle hooks, pip / Cargo build scripts, Homebrew taps, .npmrc / .pypirc credentials, Docker credential helpers, and "ClickFix" curl-pipe-shell patterns in shell history |
