...
struct Sweep: ParsableCommand {
    ...
    @Option(name: .long, help: "Run only a specific scanner (process, permission, persistence, evidence, eventtap, device, kernel, integrity, network, profile, browser, deep, hardening, clickfix, supplychain)")
    var only: String?
    ...
    func run() throws {
        ...
        if only == nil || only == "clickfix" {
            scanners.append(ClickFixScanner())
        }
