...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `applescript`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, Find My Mac, secure boot, quarantine, PATH integrity |
| **AppleScript** | Detects osascript abuse — fake password prompts, staged `.scpt` payloads, Folder-Action persistence, script applets in `/tmp` (AMOS/Banshee/Poseidon/Cthulhu/NimDoor) |
