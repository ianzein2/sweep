...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `clickfix`, `supplychain`
...
| **ClickFix** | Detects "paste-and-run" social-engineering IOCs — fake CAPTCHA payloads in shell history, recently downloaded `.command` files, osascript stagers |
| **Supply Chain** | Flags known-malicious npm/PyPI packages (Contagious Interview / BeaverTail campaign) in global installs, npm cache, and project `node_modules` |
