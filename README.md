...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, sshd config, Time Machine encryption, macOS version freshness |
| **ClickFix** | Detects evidence of fake-CAPTCHA / "verify you are human" terminal-paste attacks (the dominant macOS infection vector since 2024) by scanning shell history and Terminal saved state for `curl ... | sh`, `osascript ... administrator privileges`, `base64 -d | bash`, and similar one-liners |
