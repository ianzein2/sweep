...
struct Sweep: ParsableCommand {
    ...
    @Option(name: .long, help: "Run only a specific scanner (process, permission, persistence, evidence, eventtap, device, kernel, integrity, network, profile, browser, deep, hardening, miner, mcp)")
    var only: String?
    ...
    func run() throws {
        ...
        if only == nil || only == "miner" {
            scanners.append(CryptoMinerScanner())
        }
        if only == nil || only == "mcp" {
            scanners.append(MCPServerScanner())
        }
