...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `btm`, `ai`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH (sshd config), sharing services, software updates, Time Machine, Bluetooth |
| **Background Items** | Audits the macOS 13+ Background Tasks Management database (`sfltool dumpbtm`) — the modern replacement for LaunchAgents used by SMAppService |
| **AI Tooling** | Scans MCP server configs and hooks for Claude / Cursor / Windsurf / Continue / VS Code — detects malicious MCP entries, shell drop-and-run patterns, and prompt-injection markers |
