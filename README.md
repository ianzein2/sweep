...
|---------|-------------|
| **Process** | Matches running processes against known spyware signatures (70+ families incl. 2024-2026 DPRK / stealer campaigns), flags unsigned binaries, enumerates loaded dylibs for injection, detects orphan processes |
...
| **Evidence** | Looks for stored screenshots, keystroke logs, and recording artifacts on disk; flags live `osascript` fake password prompts, `chainbreaker` keychain extraction, pipe-to-shell loaders, and Gatekeeper-bypass `xattr` calls |
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH server config (root login / password auth), sharing services, software updates, EOL macOS version, Background Login Items (Ventura+), Bluetooth discoverability, Wi-Fi auto-join risk |
