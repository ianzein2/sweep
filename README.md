...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `shellhistory`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, macOS version currency, Signed System Volume, Advanced Data Protection |
| **Shell History** | Detects ClickFix / FakeCAPTCHA paste-and-run attacks by scanning shell history (zsh, bash, fish) for `curl | sh`, base64-decoded payloads, AppleScript password prompts, and known ClickFix filenames |
