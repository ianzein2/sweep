...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `clickfix`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, Stolen Device Protection, Background Task Management state, Wi-Fi private address, iCloud Private Relay, USB accessory policy |
| **ClickFix** | Detects fake-terminal-paste / fake-CAPTCHA attacks (the 2024-2025 delivery vector for AMOS, Banshee, FrigidStealer, and Cuckoo) by inspecting shell histories, Terminal saved state, fresh /tmp scripts, and recent quarantine-bypassed binaries |
