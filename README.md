...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `devenv`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, Background Items (SMAppService), iCloud Private Relay, Advanced Data Protection, sudo Touch ID, T2/Apple Silicon secure boot |
| **Dev Environment** | Catches dev-tool compromise (BeaverTail / Contagious Interview-style): `~/.ssh/config` ProxyCommand/LocalCommand, sshd_config drift, `.gitconfig` hooks/credential helpers, `.npmrc` / pip / `.envrc` tampering, world-readable cloud credentials, malicious editor `tasks.json`, suspicious shell history |
