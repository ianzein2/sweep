...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `credentials`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, macOS end-of-life, Find My Mac, Time Machine encryption, FileVault recovery key, XProtect Remediator recency, Secure Keyboard Entry |
| **Credential Exposure** | Surfaces plaintext credentials and wallet data that modern macOS stealers (AMOS, FrigidStealer, Banshee, Cthulhu, Odyssey) explicitly target — AWS/Docker/npm/Git/GitHub-CLI/Kubernetes/Snowflake/Databricks tokens, hard-coded API keys in editor settings, unencrypted SSH/GPG private keys, secrets in shell history, crypto wallet directories, messenger session folders |
