...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `loginitems`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, download quarantine, dangerous boot-args, macOS supportability |
| **Login Items** | Inspects modern Ventura+ login items via `sfltool dumpBTM`, app-bundle helper plists, and `~/Library/Application Scripts/` for stale, hidden, or impersonating persistence |
