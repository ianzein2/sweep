...
struct Sweep: ParsableCommand {
    ...
    @Option(name: .long, help: "Run only a specific scanner (process, permission, persistence, evidence, eventtap, device, kernel, integrity, network, profile, browser, deep, hardening, btm, ai)")
    var only: String?
    ...
    func run() throws {
        ...
        if only == nil || only == "btm" {
            scanners.append(BackgroundItemsScanner())
        }
        if only == nil || only == "ai" {
            scanners.append(AIToolingScanner())
        }
