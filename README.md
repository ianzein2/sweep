...
| **Persistence** | Scans LaunchAgents, LaunchDaemons, login items, StartupItems, rc scripts, shell configs, cron jobs, login/logout hooks, periodic scripts, Folder Action scripts, Automator workflows, SSH client ProxyCommand/LocalCommand directives |
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH server config (PermitRootLogin, PasswordAuthentication, X11Forwarding), sharing services, software updates, sudo timeout |
