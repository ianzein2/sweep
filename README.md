...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `aiagent`
...
| **AI Agent Config** | Audits Cursor/Claude Code/Windsurf/VS Code config files for auto-running commands — MCP server entries, Claude Code hooks, VS Code `runOn: folderOpen` tasks. Catches poisoned-repo and ClickFix supply-chain attacks. |
