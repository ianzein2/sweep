...
| **Persistence** | Scans LaunchAgents, LaunchDaemons, login items, StartupItems, rc scripts, shell configs, cron jobs, login/logout hooks, periodic scripts, Folder Actions / Automator agents, Background Task Manager, npm/pip package hooks, third-party Homebrew taps, and unsigned privileged helpers |
...
| **Browser** | Audits Chrome/Brave/Edge/Firefox/Safari extensions for dangerous permissions, and VSCode / Cursor / Windsurf extensions for malicious runtime patterns |
| **Deep Inspection** | Behavioral checks — root CA certificates, DNS hijacking, hidden files, ownership anomalies, DYLD environment abuse, ClickFix / FileFix paste-run patterns in shell history |
