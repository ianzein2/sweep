...
| **Persistence** | Scans LaunchAgents, LaunchDaemons, login items, StartupItems, rc scripts, shell configs, cron jobs, login/logout hooks, periodic scripts, SSH keys, sudoers, PAM, emond rules, and **plists that seed `DYLD_INSERT_LIBRARIES`** for dylib-injection persistence |
| **Evidence** | Looks for stored screenshots, keystroke logs, recording artifacts, staged crypto-wallet/credential files, **ClickFix pastejack traces in shell history**, and **exposed developer cloud credentials** (AWS/GCP/GitHub/SSH) |
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, **Time Machine backup freshness** (ransomware defense), **Gatekeeper strict mode**, **XProtect signature freshness**, and analytics/diagnostics sharing |
