...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `modern`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, Find My, Safari toggles, USB-C accessory authorization |
| **Modern Threats** | 2024-2025 threat coverage — DPRK "Contagious Interview" artifacts (BeaverTail / InvisibleFerret), malicious npm packages, reverse tunnels (ngrok / cloudflared / chisel), unsupported macOS, AI-tool credential exposure |
