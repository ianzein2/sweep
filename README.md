...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `supply`
...
| **Supply Chain** | Catches ClickFix / fake-CAPTCHA paste attacks in shell history, malicious npm postinstall hooks, known-compromised npm packages, cryptominer processes, untrusted Homebrew taps |
