...
| **Persistence** | Scans LaunchAgents, LaunchDaemons, login items, StartupItems, rc scripts, shell configs, cron jobs, login/logout hooks, periodic scripts, QuickLook generators, Spotlight importers, Application Scripts handlers (XCSSET pattern), screen savers, audio plug-ins |
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH config, sharing services, software updates, sudo timeout, Wake on Network, macOS support status |
