...
`process`, `permission`, `persistence`, `evidence`, `eventtap`, `device`, `kernel`, `integrity`, `network`, `profile`, `browser`, `deep`, `hardening`, `background`
...
| **Hardening** | CIS benchmark checks — firewall, FileVault, auto-login, screen lock, SSH, sharing services, software updates, macOS EOL, sudo timeout, Wi-Fi privacy |
| **Background Items** | Ventura+ `SMAppService` / BTM persistence — login items, agents and daemons registered without dropping a plist file, plus PrivilegedHelperTools auditing |
