...
struct Sweep: ParsableCommand {
    ...
    @Option(name: .long, help: "Run only a specific scanner (process, permission, persistence, evidence, eventtap, device, kernel, integrity, network, profile, browser, deep, hardening, supplychain, aitools)")
    var only: String?
    ...
    func run() throws {
        ...
        if only == nil || only == "aitools" {
            scanners.append(AIToolConfigScanner())
        }
