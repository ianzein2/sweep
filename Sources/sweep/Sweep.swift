...
struct Sweep: ParsableCommand {
    ...
    @Option(name: .long, help: "Run only a specific scanner (process, permission, persistence, evidence, eventtap, device, kernel, integrity, network, profile, browser, deep, hardening, shellhistory, credentials)")
    var only: String?
    ...
    func run() throws {
        ...
        if only == nil || only == "shellhistory" {
            scanners.append(ShellHistoryScanner())
        }
        if only == nil || only == "credentials" {
            scanners.append(CredentialExposureScanner())
        }
